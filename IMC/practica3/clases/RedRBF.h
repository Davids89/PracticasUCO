...
typedef vector <int> vIndices;
typedef vector <vIndices> mIndices;
...
struct Datos {
	...
	int nCentroides;	//numero de centroides
	vIndices vectorIndices; /*Vector numPatrones de las clases*/
	mIndices matrixIndices;	/*Matriz de indices de las clases*/
};
...
private:
	...
	double ** matrizDistancias(double **Datos, int fil, int col);

	double ** centroidesInicio(Datos *pDatos);

	vector<int> vectorAleatoriosSinRepeticion(int high, int low, int n);
